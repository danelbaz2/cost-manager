/* users.test.js — unit tests for the users service endpoints */
'use strict';

const request = require('supertest');
const { startDb, stopDb, clearDb } = require('./helpers/db_helper');

// Provide env vars before loading the app
...
describe('POST /api/add (user)', () => {
  test('creates a user and returns the correct fields', async () => {
    ...
    // _id and __v must not be exposed
...
describe('GET /api/users/:id', () => {
  test('returns user with total 0 when no costs exist', async () => {
    ...
    // birthday must not appear (spec only lists first_name, last_name, id, total)
